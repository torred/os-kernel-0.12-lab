...
/** step1. 复制自身到 INITSEG(0x9000:0000) */
    movw    $BOOTSEG, %ax
    movw    %ax, %ds            /* 将ds段寄存器设置为0x07C0，引导代码当前所处地址（源地址） */
    movw    $INITSEG, %ax
    movw    %ax, %es            /* 将es段寄存器设置为0x9000,引导代码要被复制到的目的地址 */
    movw    $256, %cx           /* movsw 一次移动1个字，总共移动次数为 512字节/2 = 256个字，循环256次 */
    subw    %si, %si            /* 源地址偏移si清零，ds:si = 0x07C0:0x0000，ds:si物理地址0x07c00 */
    subw    %di, %di            /* 目标地址di清零， es:si = 0x9000:0x0000，es:di物理地址0x90000 */
 ...
 */
go:
    movw    %cs, %ax            /* 跳转到移动后代码执行的代码段寄存器cs = 0x9000 */
    movw    %ax, %ds
    movw    %ax, %es
    /* put    stack at 0x9ff00  , 此指令已不支持 */
    movw    %ax, %ss
    movw    $0xFF00, %sp        /* arbitrary value >>512 # 栈空间的起始地址为0x9ff00(ss:0x9000,sp:0xFF00) */
 ...
 */
load_setup:
    movw    $0x0000, %dx            /* drive 0, head 0 # dh=磁头号，dl=驱动器号(硬盘则7要置位) */
    movw    $0x0002, %cx            /* sector 2, track 0 # ch=磁道(柱面)号的低八位   cl＝开始扇区(位0-5),磁道号高2位(位6－7) */
    movw    $0x0200, %bx            /* es:bx = 0x9000:0200，紧接着bootsect 0x9000:0x01FF(1个扇区512B大小) */
    movw    $(0x0200+SETUPLEN), %ax /* service 2, nr of sectors # ah=0x02 读磁盘扇区到内存   al＝需要读出的扇区数量 */
    ...
    movw    $0x0000, %dx            /* dx:需要复位的驱动器信息 */
    movw    $0x0000, %ax            /* reset the diskette # cf!=０中断出错复位dx指定的驱动器 */
 ...
 * es:di 软驱磁盘参数表.
 */
    movb    $0x00, %dl          /* floppy # DL＝驱动器，00H~7FH：软盘；80H~0FFH：硬盘 */
    movw    $0x0800, %ax        /* AH=8 is get drive parameters # ah=0x08 读取磁盘参数 */
    int     $0x13
    /* mov  $0x00, %cx
    /* seg  cs                  , 下一条语句的操作数在cs所指段中 */
    andw    $0x003f, %cx        /* 获取cx中 0-5 位 = 每磁道最大扇区数 */
    movw    %cx, %cs:sectors+0  /* %cs means sectors is in %cs # 保存每磁道扇区数 */
    movw    $INITSEG, %ax
    movw    %ax, %es            /* 为下面int 10h答应字符设置字符串地址所在段 $INITSEG（0x9000） */

/* 通过设置显示模式达到清屏效果 */
    movw     $0x0003, %ax        /* AH = 00H 设定显示模式；AL = 03H 文字模式，分辨率 80*25，颜色 16 */
 ...
 * bl = 字符属性. dh = 行号; dl = 列号.
 */
    movb    $0x03, %ah          /* read cursor pos # ah=0x03指示读光标位置 */
    xorb    %bh, %bh
    int     $0x10               /* dh(row 0-24) dl(col 0-79) # 0x10号中断,读取光标位置，DH = 光标行数，DL = 光标列数 */

    movw    $32, %cx            /* nr of characters # CX = 串长度 */
    movw    $0x000b, %bx        /* page 0, attribute b，attribute 7 (normal) # BH = 页号， BL = 属性 */
    /* lea  msg1, %bp */
    movw    $msg1, %bp          /* 指向要显示字符串的地址,即显示在屏幕上的第一个string "linux 0.12 OS is booting ..." */
    movw    $0x1301, %ax        /* write string, move cursor # ah=0x13 输出一个字符，移动一下光标 */
 ...
 */
    movw    $SYSSEG, %ax
    movw    %ax, %es            /* segment of 0x01000 # 内核代码加载段地址 es = 0x1000 */
 ...
 */
    ...
    /* seg cs */
    movw    %cs:root_dev+0, %ax     /* ax = ROOT_DEV（0x301） */
    cmp     $0, %ax
    jne     root_defined            /* 如果ROOT_DEV不等于0，则跳转到 root_defined */
    /* seg  cs */
    movw    %cs:sectors+0, %bx      /* 取磁道扇区数，如果sectors==15,则说明是1.2Mb驱动器 */
...
root_defined:
    /* seg  cs */
    movw    %ax, %cs:root_dev+0     /* 将检查过的设备号保存到 root_dev 中 */
...
read_it:
    movw    %es, %ax
    testw   $0x0fff, %ax            /* 测试es段地址是否在4k边界，最终system模块加载内存地址边界64KB边界(0x1000 << 4 = 0x10000) */
die:
    jne     die                     /* es must be at 64kB boundary # 如果不是则进入死循环 */
    xorw    %bx, %bx                /* bx is starting address within segment # 段内偏移，清零 bx = 0 */
rp_read:                            /* 重复读system模块 */
    movw    %es, %ax
    cmp     $ENDSEG, %ax            /* have we loaded all yet? # ax - ENDSEG，看是否到了末尾, */
                                    /* ENDSEG=SYSSEG+SYSSIZE=0x10000+0x30000=0x40000,SYSSIZE=0x3000*0x10=192kB */
    jb      ok1_read                /* ax >= 0x40000则返回 */
    ret
ok1_read:
    #seg    cs
    movw    %cs:sectors+0, %ax      /* 每磁道总扇区数 */
    subw    sread, %ax              /* ax = ax - sread，得出本磁道未读扇区数 */
    movw    %ax, %cx
    shlw    $9, %cx                 /* 右移9位，计算剩余扇区的总字节数 总字节数 = 扇区数cx << 9 = 扇区数cx * 512 */
    addw    %bx, %cx                /* 偏移地址累加将要读取的字节数 cx = ax * 512 + bx */
    jnc     ok2_read                /* 若cx < 0x10000（CF=0,没有进位）则跳转到ok2_read，不会越界 */
    je      ok2_read                /* 若cx = 0（ZF=1），说明刚好不越界，则跳转到ok2_read */
    xorw    %ax, %ax                /* 执行到这里说明越界，令 ax = 0x0000 */
    subw    %bx, %ax                /* 计算bx离边界有多远，用0减去bx，结果在ax中 */
    shrw    $9, %ax                 /* 越界后，只读到越界的扇区，读取字节数 = 扇区数ax << 9 = 扇区数 * 512 */
ok2_read:
    call    read_track              /* 调用read_track过程，用AL传参,读取AL个扇区到ES:BX */
                                    /* read_track 中有出错检测，能返回，说明读取成功（AH=0） */
    movw    %ax, %cx                /* AL中是返回值，即实际读到的扇区数目，cx是该次操作已经读取的扇区数 */
    addw    sread, %ax              /* ax是当前磁道已经读取的扇区数 */
    #seg    cs
    cmp     %cs:sectors+0, %ax
    jne     ok3_read                /* 如果当前磁道还有扇区未读，跳转到ok3_read */
    movw    $1, %ax                 /* 说明当前磁道的扇区数已读完 */
    subw    head, %ax               /* ax = 1 - 磁头号 */
    jne     ok4_read                /* 不相等说明磁头号为0，跳转到 ok4_read，ax=1,读完0磁头，再读1磁头 */
    incw    track                   /* 说明磁头号为1，ax=0,设置磁头号为0，磁道号增加1 */
ok4_read:
    movw    %ax, head               /* 保存当前磁头号 */
    xorw    %ax, %ax                /* ax=0, 当前磁道已读扇区数置0 */
ok3_read:
    movw    %ax, sread              /* 更新当前磁道已经读取的扇区数 */
    shlw    $9, %cx                 /* cx当前磁道已经读取的扇区数，乘以512得当前磁道已经读取字节数 */
    addw    %cx, %bx                /* 更新偏移地址 */
    jnc     rp_read                 /* 没有进位则跳转到rp_read继续读 */
    movw    %es, %ax                /* 有进位，说明bx达到了64kB边界 */
    addw    $0x1000, %ax
    movw    %ax, %es                /* es增加0x1000 */
    xorw    %bx, %bx                /* 偏移地址清零 bx = 0x0 */
    jmp     rp_read                 /* 继续读取 */
...
read_track:
    ...
    movw    head, %dx               /* 当前磁头号 */
    movb    %dl, %dh                /* DH是磁头号 */
    movb    $0, %dl                 /* DL是驱动器号，0表示软盘 */
    andw    $0x0100, %dx            /* DH是磁头号，磁头号不大于 1 */
    movb    $2, %ah                 /* 功能号2，读扇区 */
...
bad_rt:
    movw    $0, %ax                 /* AH=0，磁盘复位功能 */
    movw    $0, %dx                 /* DL是驱动器号，0表示软盘 */
    int     $0x13
    popw    %dx
    popw    %cx
    popw    %bx
...
kill_motor:
    pushw   %dx
    movw    $0x3f2, %dx             /* 软盘控制器的端口-数字输出寄存器端口,只写 */
    movb    $0, %al                 /* 驱动器A,关闭FDC,禁止DMA和中断请求,关闭马达 */
    outsb                       /* 将al的值写入端口dx */
    popw    %dx
    ret

...
/** print some message */
    ...
    movw    $27, %cx
    movw    $0x000b,%bx
    movw    $msg2,%bp           /* es:bp = msg2 "Now we are in setup ..." */
    movw    $0x1301, %ax
    int     $0x10
 ...
 */
    ...
    movw    %ax, %ds            /* ds设置成INITSET */
    movb    $0x03, %ah          /* read cursor pos #int 10读光标功能号 3，读取光标位置 */
    xorb    %bh, %bh
    int     $0x10               /* save it in known place, con_init fetches #调用中断，读取光标位置，con_init需要使用 */
    movw    %dx, %ds:0          /* it from 0x90000. # 光标信息存在dx中，dh = 行，dl = 列，存入0x90000处 */
 ...
 */
    movw    $INITSEG, %ax
    movw    %ax, %ds
    movb    $0x88, %ah          /* int 15取扩展内存大小功能号0x88 */
    int     $0x15
    movw    %ax, %ds:2          /* save the value of extended memory size in 0x90002, */
 ...
 */
    ...
    movb    $0x0f, %ah
    int     $0x10
    movw    %bx, %ds:4          /* bh = display page (0x00) #存入0x90004 */
    movw    %ax, %ds:6          /* al = video mode, ah = window width (0x50,0x03)  # #存入0x90006 */
                                /* 80x25(col*row) */
/**
 ...
 */
    movw    $INITSEG, %ax
    movw    %ax, %ds
    movb    $0x12, %ah          /* 0x12功能号，显示器的配置中断 */
    movb    $0x10, %bl          /* 0x10H子功能，读取配置信息 */
    int     $0x10
    movw    %ax, %ds:8          /* ax存入0x90008, 调用中断后ax = 0x1200, 把此值存下来不清楚什么用？？ */
    movw    %bx, %ds:10         /* bx存入0x9000a */
                                /* bh = video mode, bh = 0，单色模式；1，彩色模式 */
                                /* bl = VRAM容量, bl = 00h->64K, 01h->128H，02h->192K, 03h->256K */
    movw    %cx, %ds:12         /* cx存入0x9000c, ch:特征连接器标志位; cl:EGA开关设置(EGA 80x25) */
    movw    $0x5019, %ax        /* 80x25 */
    movw    %ax, %ds:14         /* 存入0x9000e */
 ...
 */
    movw    $0x0000, %ax
    movw    %ax, %ds
    ldsw    %ds:4*0x41, %si     /* src #4*0x41是中断向量41在中断向量表中的偏移地址，取中断向量41的值，中断向量0x41保存的是硬盘0参数表的地址 */
                                /* lds命令：ds:4*0x46 -> si，ds:4*0x46+2 -> ds */
    movw    $INITSEG, %ax
    movw    %ax, %es
    movw    $0x0080, %di        /* dst: 0x90080 # 传输向量表到达目的地址 es:si -> 9000:0080 */
    movw    $0x10, %cx          /* the size of FDPT is 16) 取16个字节 */
    rep
    movsb
 ...
 */
    movw    $0x0000, %ax
    movw    %ax, %ds
    ldsw    %ds:4*0x46, %si     /* 取中断向量46的值，即硬盘1参数表的地址 */
    movw    $INITSEG, %ax
    movw    %ax, %es
    movw    $0x0090, %di        /* 传输向量表到达目的地址 es:di -> 9000:0090 */
    movw    $0x10, %cx          /* 取16个字节 */
    rep
    movsb
...
/** show cursor pos #显示当前光标位置 */
    ...
    movw    $11,%cx
    movw    $0x000a,%bx         /* 设置显示文字属性 */
    movw    $cur,%bp
    movw    $0x1301,%ax
    int     $0x10               /* 显示光标信息的提示文字 */
/** show detail(row*col) */
    movw    %ds:0 ,%ax          /* 需要打印的光标位置 */
...
/** show nr of sectors per track, 显示每磁道扇区数 */
    ...
    int     $0x10
/** show detail */
    movw    %ds:0x8e, %ax       /* 需要打印的第一个硬盘每磁道扇区数 */
    call    print_hex
    call    print_nl
...
do_move:
    movw    %ax, %es            /* destination segment # 此时ax=0x0000，被复制的目的地址es:di -> 0x0000:0000 */
    addw    $0x1000, %ax        /* 每次移动0x1000字节(64KB) */
    cmpw    $0x9000, %ax        /* 0x9000 SYS_SIZE */
    jz      end_move
    movw    %ax, %ds            /* source segment # 原地址ds:si -> 0x1000:0 0x2000:0 0x3000:0··· */
    subw    %di, %di            /* di = 0，实模式下最大偏移是64k（0x0-0xFFFF，共0x10000h=64kb）,所以一次最多复制这么多 */
    subw    %si, %si            /* 第一次放在0-0x10000处，第二次0x10000-0x20000··· */
    movw    $0x8000, %cx        /* 总共移动了0x8000(32768)字,即0x10000Byte,(65536/1024=64k) */
...
end_move:
    ...
    movw    %ax, %ds            /* 0x9020 # ds指向setup段0x9020 */
    ...
    lgdt    gdt_48              /* load gdt with whatever appropriate # 加载全局描述符表寄存器 */
                                /* gdt_48: 0x0009，0x0200+gdt，0x07FF，limit=2048 base = 0x9xxxx */
...
/** 设置8259中断控制器 */
    movb    $0x11, %al          /* initialization sequence(ICW1) */
    ...
    movb    $0x20, %al          /* start of hardware int's (0x20)(ICW2) */
    outb    %al, $0x21          /* from 0x20-0x27 */
    .word   0x00eb,0x00eb

    movb    $0x28, %al          /* start of hardware int's 2 (0x28) */
    outb    %al, $0xA1          /* from 0x28-0x2F */
    .word   0x00eb,0x00eb       /* IR 7654 3210 */

    movb    $0x04, %al          /* 8259-1 is master(0000 0100) */
    outb    %al, $0x21
    .word   0x00eb,0x00eb       /* INT */

    movb    $0x02, %al          /* 8259-2 is slave(010 --> 2) */
    outb    %al, $0xA1
    .word   0x00eb,0x00eb
...
/** 开启保护模式 */
    ...
    /* lmsw   %ax               # This is it! */
    movl    %cr0, %eax          /* get machine status(cr0|MSW) */
    btsl    $0, %eax            /* turn on the PE-bit, cr0的第0位为PE位，置此位为1 */
    movl    %eax, %cr0          /* protection enabled */
...
gdt_48:
    ...
    .word   512+gdt, 0x9        /* gdt base = 0X9xxxx  # base = 0x0009 0200+$gdt -> 0x9xxxx，表示从90000开始,偏移bootsect */
                                /* 512字节，再偏移gdt在setup的偏移，得到gdt在内存中的物理地址 */
...
print_digit:
    rolw    $4,%dx              /* 循环以使低4位用上，高4位移至低4位 */
    movw    $0xe0f,%ax          /* ah ＝ 请求的功能值，al = 半个字节的掩码 */
    andb    %dl,%al
    addb    $0x30,%al
    cmpb    $0x3a,%al
    jl      outp

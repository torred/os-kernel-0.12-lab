 ...
 *	14(%esp) - %es              18(%esp) - %es
 *	18(%esp) - %ds              1C(%esp) - %ds
 *	1C(%esp) - %eip             20(%esp) - %eip
 *	20(%esp) - %cs              24(%esp) - %cs
 *	24(%esp) - %eflags          28(%esp) - %eflags
 *	28(%esp) - %oldesp          2C(%esp) - %oldesp
 *	2C(%esp) - %oldss           30(%esp) - %oldss
 *
...
nr_system_calls = 88				/* Linux 0.12版内核中的系统调用总数. */
...
system_call:
	...
	/* 在保存过段寄存器之后,让ds,es指向内核数据段,而fs指向当前局部数据段,即指向执行本次系统调用的用户程序的数据段. */
	/* 注意, 在Linux0.12中内核给任务分配的代码和数据内存段是重叠的,它们的段基址和段限长相同. */
 ...
 * 但是其时间片已经用完(counter=0),则也去执行调度程序.
...
device_not_available:
	...
	movl $0x17, %eax				/* fs置为指向局部数据段(出错程序的数据段). */
	mov %ax, %fs
	/* 清CR0中任务已交换标志TS,并取CR0值.若其中协处理器仿真标志EM没有置位,说明不是EM引起的中断,则恢复任务协处理器状态, */
	/* 执行C函数math_state_restore(),并在返回时去执行ret_from_sys_call处的代码. */
...
timer_interrupt:
	...
	pushl $-1						/* fill in -1 for orig_eax	# 填-1,表明不是系统调用. */
	/* 下面我们保存寄存器eax,ecx和edx.这是因为gcc编译器在调用函数时不会保存它们.这里也保存了ebx寄存器. */
	/* 因为在后面ret_from_sys_call中会用到它. */
...
1:	jmp 1f
	/* do_hd定义为一个函数指针,将被赋值read_intr()或write_intr()函数地址.放到edx寄存器后就将do_hd指针变量置为NULL. */
	/* 然后测试得到的函数指针, 若该指针为空,则赋予该指针指向C函数unexpected_hd_interrupt(),以处理未知硬盘中断. */
1:	xorl %edx, %edx

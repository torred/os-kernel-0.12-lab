...
/**
 * PC8253定时芯片的输入时钟频率约为1.193180MHz.Linux内核希望定时器发出中断的频率是100Hz,也即每10ms发出一次时钟中断.
 * 因此这里LATCH是设置8253芯片的初值.
 */
...
/** 设置初始任务0的数据, 初始数据在include/kernel/sched.h中. */
...
/** 这个变量用于累计需要调整的时间滴答数
 * clock ticks to add to get "true time".  Should always be less than
 * 1 second's worth.  For time fanatics who like to syncronize their machines to WWV :-)
 */
 ...
 * ss被设置为内核数据段选择符(0x10),指针esp指在user_stack数组最后一项后面.这是因为Intel CPU执行堆栈操作时是先递减堆栈指针sp值,
 * 然后在sp指针处保存入栈内容
 */
long user_stack [ PAGE_SIZE*2 >> 2 ];	/* 2022.10.31 modify:在MacOS下出现死循环问题修改，原因是user_stack太小，导致esp与.bss的vc_cons内存重叠 */
...
void schedule(void)
{
	...
	for(p = &LAST_TASK ; p > &FIRST_TASK ; --p)
		if (*p) {
			/* 如果设置过任务超时定时timeout,并且已经超时,则复位超时定时值,并且如果任务处于可中断睡眠状态TASK_INTERRUPTIBLE下 */
	...
	while (1) {
		...
		i = NR_TASKS;
		p = &task[NR_TASKS];
		/* 这段代码是从任务数组的最后一个任务开始循环处理,并跳过不含任务的数组糟.比较每个就绪状态任务的counter */
		/* (任务运行时间的递减滴答计数)值,哪一个值大,运行时间还不长,next就指向哪个的任务号 */
		while (--i) {
			/* 当前索引没有进程指针则跳过当前循环 */
			if (!*--p)
				continue;
		...
		/* 如果比较得出有counter值不等于0的结果,或者後方中没有一个可运行的任务存在(此时c仍然为-1,next=0),则退出开始的循环, */
		/* 执行161行上的任务切换操作.否则就根据每个任务的优先权值,更新每一个任务的counter值,然后回到125行重新比较. */
		/* counter值的计算方式为counter = counter /2 +priority. 注意,这里计算过程不考虑进程的状态. */
		if (c) break;
		for(p = &LAST_TASK ; p > &FIRST_TASK ; --p)
			if (*p)
	...
	/* 用下面的宏(定义在sched.h中)把当前任务指针current指向任务号为next的任务,并切换到该任务中运行.在146行上next被初始化为0. */
	/* 因此若系统中没有任何其他任务可运行时,则next始终为0.因此调度函数会在系统空闲时去执行任务0.此时任务0权执行pause() */
...
static inline void __sleep_on(struct task_struct **p, int state)
{
	...
	/* 这样就把当前任务插入到了*p的等待队列中.然后将当前任务置为指定的等待状态,并执行重新调度. */
	tmp = *p;
	*p = current;
	current->state = state;
...
repeat:	schedule();
	/* 只有当这个等待任务被唤醒时,程序才会返回到这里,表示进程已被明确地唤醒并执行.如果等待队列中还有等待任务,并且队列头指针*p所指向的任务不是当前任务时, */
    /* 说明在本任务插入等待队列后还有任务进入等待队列.于是我们应该也要唤醒这个任务,而我们自己应按顺序让这些后面进入队列的任务唤醒,因此这里将等待队列头所 */
    /* 指任务先置为就绪状态,而自己则置为不可中断等待状态,即自己要等待这些后续队列的任务被唤醒而执行时来唤醒本任务.然后重新执行调度程序. */
	if (*p && *p != current) {
		(**p).state = 0;
...
/* 唤醒*p指向的任务.*p是任务等待队列头指针.由于新等待任务是插入在等待队列头指针处的,因此唤醒的是最后进入等待队列的任务. */
...
int ticks_to_floppy_on(unsigned int nr)
{
	...
	/* 系统最多有4个软驱.首先预先设置好指定软驱nr停转之前需要经过的时间(100秒).然后取当前DOR寄存器值到临时变量mask中, */
	/* 并把指定软驱的马达启动标志置位. */
	if (nr > 3)
...
void do_timer(long cpl)
{
	...
	/* 首先判断是否经过了一定时间而让屏幕黑屏(blankcount).如果blankcount计数不为零,或者黑屏延时间隔时间blankinterval为0的话, */
	/* 那么若已经处理黑屏状态(黑屏标志blanked=1),则让屏幕恢复显示.若blnkcount计数不为零,则递减之,并且复位黑屏标志. */
	...
	/* 如果有定时器存在,则将链表第1个定时器的值减1.如果已等于0,则调用相应的处理程序,并将该处理程序指针置空. */
	/* 然后去掉该项定时器.next_timer是定时器链表的头指针 */
	if (next_timer) {
		next_timer->jiffies--;
...
/**
 * 系统调用功能 - 设置报警定时时间值(秒),若参数seconds大于0,则设置新定时值,并返回原定时时刻还剩余的间隔时间,否则返回0.
...
void sched_init(void)
{
	...
	/* Linux系统开发之初,内核不成熟.内核代码会被经常修改.Linus怕无意中修改了这些关键性的数据结构,造成与POSIX标准的不兼容. */
	...
	/* 实际上他也对应程序head.s中的描述符表基址(gdt).因此gdt+FIRST_TSS_ENTRY即为gdt[FIRST_TSS_ENTRY](即是gdt[4]),即gdt数组第4项的地址 */
	/* 参见include/asm/system.h */
	set_tss_desc(gdt + FIRST_TSS_ENTRY, &(init_task.task.tss));		/* 设置0号进程的tss_desc: gdt + 4 */
	set_ldt_desc(gdt + FIRST_LDT_ENTRY, &(init_task.task.ldt));		/* 设置0号进程的ldt_desc: gdt + 5 */
	/* 清任务数组和描述符表项(注意i=1开始,所以初始任务的描述符还在).描述符项结构定义在文件include/linux/head.h中 */
	p = gdt + 2 + FIRST_TSS_ENTRY;									/* p -> gdt + 6 */

	for(i = 1; i < NR_TASKS; i++) {
		task[i] = NULL;										/* task[0]为进程0, 从task[1]开始初始化, 一直到task[63](NR_TASKS=64) */
		p->a = p->b = 0;									/* 置 gdt+6+i-1 -> tss = 0, 置63个tss */
		p++;
		p->a = p->b = 0;									/* 置 gdt+7+i-1 -> ldt = 0 置63个ldt */
		p++;
	}

	/**
	 * Clear NT, so that we won't have troubles with that later on
	 * 清除标志寄存器中的位NT,这样以后就不会有麻烦, EFLAGS中的NT标志位用于控制任务的嵌套调用.
	 * 当NT位置位时,那么当前中断任务执行IRET指令时就会引起任务切换.NT指出TSS中的back_link字段是否有效.
	 * NT=0时无效.
	 */

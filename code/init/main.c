 ...
 * fork()系统调用函数的定义
 */
_syscall0(int, fork)                                // pause()系统调用函数的定义
_syscall0(int, pause)                               // int setup(void * BIOS)系统调用,仅用于linux初始化(仅在这个程序中被调用).
_syscall1(int, setup, void *, BIOS)                 // int sync()系统调用：更新文件系统。
_syscall0(int, sync)
...
{                                                   /* The startup routine assumes (well, ...) this */
    ...
    chr_dev_init();                                  /* 字符设备初始化, 函数为空.(chr_drv/tty_io.c) */
    ...
    /* 下面过程通过在堆栈中设置的参数,利用中断返回指令启动任务0执行, 同时从内核态切换到用户态运行,操作系统规定进程都必须运行在3特权上 */
    move_to_user_mode();                             /* 切换到用户模式下执行.(include/asm/system.h) */

...
// 物理内存管理初始化.
// 该函数对1MB以上内存区域以页为单位进行管理，并对内存映射数组进行初始化。一个内存页长4KB，该函数对1MB以上所有物理内存划分成一个个页,并
// 使用一个页面映射字节数组mem_map[]来管理所有这些页。对于16MB内存的机器,该数组共有3840项((16MB-1MB)/4KB),即可管理3840
// 个物理页.每当一个物理内存页被占用时，次页在mem_map[]中对应的字节值增1；若释放一个物理页就把对应字节值减1。若字节值为0,则表示对应页面空闲;
// 若字节值大于或等于1，则表示对应页被占用或被不同程序共享占用。在该版本的Linux内核中最多能管理16MB物理内存,大于16MB的内存将弃置不用。
// 对于具有16MB内存的PC系统,在没有设置虚拟盘RAMDISK的情况下,共有3072个物理页面可供分配.而范围0~1MB内存空间用于内核系统(其实内核
// 只使用0~640KB,剩下的部分被部分高速缓冲和设备内存占用).
// 参数start_mem是可用作页面分配的主内存区起始地址(已去除RAMDISK所占内存空间).end_mem是实际物理内存最大地址.而地址范围start_mem到
// end_mem是主内存区.
// 无RAMDISK: start_mem = 0x400000; 有RAMDISK: start_mem = 0x480000
// end_mem = 0x1000000
void mem_init(long start_mem, long end_mem)
{
	...
	for (i = 0; i < PAGING_PAGES; i++)
		mem_map[i] = USED;
	// 然后计算主内存区起始内存start_mem处页面对应内存映射字节数组中项号i和主内存区页面数.此时mem_map[]数组的第i项正对应主内存区中第i个页面.
	// 最后将主内存区中页面对应的数组项清零(表示空闲).对于具有16MB物理内存的系统,mem_map[]中对应4MB~16MB主内存区的项被清零.
	i = MAP_NR(start_mem);									// 主内存区起始位置处页面号,i = 768
	end_mem -= start_mem;
	// 得到主内存区的页面的数量
	end_mem >>= 12;											// 主内存区中的总页面数，end_mem = 3072
	// 将主内存区对应的页面数的应用数置零
	while (end_mem-- > 0)
		mem_map[i++] = 0;									// 主内存区页面对应字节值清零，mem_map[768..3839] = 0
}

// 显示系统内存信息
// 根据内存映射字节数组mem_map[]中的信息以及页目录和页表内容统计系统中使用的内存页面数和主内存区中总物理内存页面数.该函数在chr_drv/keyboard.S程序
// 被调用，即当按下"Shift + Scroll Lock"组合键时会显示系统内存统计信息.
void show_mem(void)
{
